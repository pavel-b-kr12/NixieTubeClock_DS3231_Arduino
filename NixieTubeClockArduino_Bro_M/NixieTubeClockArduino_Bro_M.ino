...
const int  btn_swap_time_date =	2; //btn show date	//Выводы (если свободны)  для кнопок и транзистора|реле-модуля подсветки:
#define  btn_backlight 			5 //btn backlight
const int  backlight_out =		10; //out for N ch FET for backlight
...
bool bShow = 0; // флаг для мигания цифр при установке времени и светодиода (двоеточие)
...
int digits[5]; // массив для текущего значения времени на ШЕСТЬ цифр
int digits_last[5];
...
void loop() {
	if (millis()> nextReadRTC) // если прошло 500мс
	{
		...
		bShow=!bShow; // инвертируем флаг мигающей цифры
		digitalWrite(LED_colon,bShow);

		ReadDS3231(); 
	}
	
	bool bDateShow_last=bDateShow;
	if(millis()>bDateShow_untill_t)
	{
		#ifndef DateShow_constant
		if(bDateShow && mode=mode_show) bDateShow=false;
		#endif
		
		if(!digitalRead(btn_swap_time_date))
		{
			bDateShow_untill_t=millis()+4000;
	...
	if(bDateShow!=bDateShow_last) memcpy(digits_last, digits, 6*sizeof(int));
	
	#ifdef btn_backlight
	if(!digitalRead(btn_backlight))
	{
		bbacklight_untill_t=millis()+10000;
	}
	#endif
	digitalWrite(backlight_out, bBacklight 
	#ifdef btn_backlight
	|| (millis()<bbacklight_untill_t)
	#endif
	);
	...
	*/
		...
		else //{if (keynum==key_Sub || keynum==key_Add_BacklightSw)}
		{
			...
			case mode_change_pos: 
				if(currentdigit==0 && incr==-1 )currentdigit=2;
				else
				{
					currentdigit+=incr; 
			...
			case mode_change_value: 
				bShow=true; // прекращаем мигать цифрами
	...
	if(bDateShow || millis()<bDateShow_untill_t)
	{
		digits[0] = date / 10;
		digits[1] = date  % 10;
		digits[2] = month / 10;
		digits[3] = month % 10;
		// digits[4] = (year%100) / 10; //!TST
		digits[4] = year / 10;
		digits[5] = year % 10;
	}
	else
...
void show(int hhmmss[])
{
	setNumber(hhmmss[0]); //выведем цифру hhmmss[0] на первый индикатор
	if(!( (mode==mode_change_pos) && (currentdigit==0) && !bShow) )	showDigit(key1); //если мы в режиме настройки и происходит настройка часов, то в первая цифра будет мигать

	setNumber(hhmmss[1]); //цифра hhmmss[1] на второй индикатор
	if(!( (mode==mode_change_value) && (currentdigit==0) && !bShow))	showDigit(key2);

	setNumber(hhmmss[2]);
	if(!( (mode==mode_change_pos) && (currentdigit==1) && !bShow))		showDigit(key3);

	setNumber(hhmmss[3]);
	if(!( (mode==mode_change_value) && (currentdigit==1) && !bShow))	showDigit(key4);

	setNumber(hhmmss[4]);
	if(!( (mode==mode_change_pos) && (currentdigit==2) && !bShow) )		showDigit(key5);

	setNumber(hhmmss[5]);
	if(!( (mode==mode_change_value) && (currentdigit==2) && !bShow))	showDigit(key6);
}
